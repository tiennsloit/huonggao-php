...
function show_image ($mime_type, $image_resized, $quality, $cache_dir, $new_width) {
	...
	$d->reset();
	$sql_dongdau = "select photo from table_background where type='dongdau' limit 0,1";
	$d->query($sql_dongdau);
	...
	//imagesy($watermark) Chiều cao hình đóng dấu
 	$watermark_pos_x = imagesx($image_resized) - imagesx($watermark) - 5;
    $watermark_pos_y = imagesy($image_resized) - imagesy($watermark) - 5;
