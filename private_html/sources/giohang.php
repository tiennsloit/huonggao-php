<?php  if(!defined('_source')) die("Error");	
	...
	#Lấy tỉnh thành phố
	$d->reset();
	$sql = "select id,ten from #_place_city where hienthi=1 order by stt,id asc";
	$d->query($sql);
		...
		if(mysql_query($sql))
		{
			if(is_array($_SESSION['cart']))
			{
				...
				{		
					...
					$mail->AddAddress($company['email'], 'Đơn hàng từ website '."localhost");
					$mail->AddAddress($email, 'Đơn hàng từ website '."localhost");
					//Thiết lập email nhận email hồi đáp
					
					//nếu người nhận nhấn nút Reply
					$mail->AddReplyTo($email,'Đơn hàng từ website'."localhost");
					/*=====================================
					 * THIET LAP NOI DUNG EMAIL
					*=====================================*/
					//Thiết lập tiêu đề
					$mail->Subject    = "Đơn hàng từ website"."localhost";
					$mail->IsHTML(true);
					...
					$mail->CharSet = "utf-8";		
						...
						$body .= '
							...
							<tr>
								<th colspan="2">Đơn hàng từ website <a href="'."localhost".'">'."localhost".'</a></th>
							</tr>
							<tr>
								<th colspan="2">&nbsp;</th>
							</tr>
