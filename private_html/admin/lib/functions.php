...
function checkPermission(){
	global $d;
	$sql = "select secretkey from #_user where id='".$_SESSION['login_admin']['id']."'";
	$d->query($sql);
	$row = $d->fetch_array();
	if($_SESSION['login_admin']['secretkey']){
		if(!empty($row) && $_SESSION['login_admin']['secretkey']==$row['secretkey']){
			return true;
		}else{
			session_unset();
